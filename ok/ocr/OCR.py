...
class OCR:
    ...
    def ocr(self, x=0, y=0, to_x=1, to_y=1, match: str | List[str] | Pattern[str] | List[Pattern[str]] | None = None,
        ...
        if image is None:
            ...
            communicate.emit_draw_box("ocr_zone" + join_list_elements(name), box, "blue")
            if log:
                logger.info(
            ...
            for detected_box in detected_boxes:
                if detected_box.name in self.text_fix:
                    detected_box.name = self.text_fix[detected_box.name]
...
def join_list_elements(input_object):
